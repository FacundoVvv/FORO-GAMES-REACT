...
export const Header = () => {
  ...
  const [isAccountDropdownOpen, setAccountDropdownOpen] = useState<boolean>(false);
  const { user } = useAuth();
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        !accountLinkRef.current.contains(event.target as Node) &&
  ...
  return (
    <header className="sticky top-0 z-50 w-[100%] bg-gradient-to-r from-purple-600 via-purple-500 to-indigo-600 shadow-lg">
