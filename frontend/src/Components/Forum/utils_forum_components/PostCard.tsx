...
export const PostCard = ({ post, setSelectedPost, section }: PostCardProps) => {
  const authorName = post.author?.username ?? "Usuario desconocido";
  const titleText = post.title ?? "Sin título";
  ...
  return (
    ...
    >
      <HiDocumentText className="text-4xl text-indigo-800 col-start-1 col-end-2 row-start-1 row-end-2 self-center" />
      {post.author?.username ? (
        <Link
          to={`/forum/user/${authorName}`}
          className="text-left font-semibold text-sm text-indigo-900 col-start-2 col-end-3 row-start-1 row-end-2 self-center"
        >
          {authorName}
        </Link>
      ) : (
        <span className="text-left font-semibold text-sm text-indigo-900 col-start-2 col-end-3 row-start-1 row-end-2 self-center">
          {authorName}
        </span>
      )}
      ...
      >
        <h2 className="text-center text-base font-bold tracking-wider text-gray-900 break-words w-full">
          {titleText}
        </h2>
      </div>
      <div className="col-start-1 col-end-4 row-start-3 row-end-4 w-full flex-col items-center justify-center h-full">
        <p className="line-clamp-3 text-base text-gray-700 break-words h-auto">
          {descriptionText}
        </p>
        {descriptionText.length > 105 && (
          <button
